...
public class BackwardChain {
	...
	public BindingList unify(Term t1, Term t2, BindingList bl) {
		...
		// t1 is a variable
			...
			return unifyVariable(t1.v, t2, bl);
		}
		// t2 is a variable
		else if (t2.v != null) {
			return unifyVariable(t2.v, t1, bl);
		}
		// t1 and t2 are functions
	...
	public BindingList unifyVariable(Variable var, Term term, BindingList bl) {
		// var is in binding list
		if (bl.groundValue(var) != null) {
			return unify(bl.groundValue(var), term, bl);
		} else if (occurs(var, term.allVariables())) { // variable v1 occurs in term v2
			return (null);
		} else { // add binding
			bl.addVariableBinding(var, term);
			return new BindingList(bl);
		}
	}

	// checks if var occurs in set of variables
	public boolean occurs(Variable var, Set<Variable> variables) {

...
public class Layer {
	...
	public void computeOutputDelta() {
		Vector derivativeVector = net.derivative(min, max);
		// For each node in output layer
		for (int i = 0; i < n; i++) {
			// Compute output delta
			delta.set(i, derivativeVector.get(i) * (targ.get(i) - act.get(i)));
		}
	}
	...
	public void computeHiddenDelta() {
		Vector sums = new Vector(n);
		// For each projection in outputs
		for (Projection p : outputs) {
			// Sum the transpose product of delta
			sums = sums.sum(p.W.transpose().product(p.output.delta));
		}
		Vector derivativeVector = net.derivative(min, max);
		// For each node in output layer
		for (int i = 0; i < n; i++) {
			// Compute output delta
			delta.set(i, derivativeVector.get(i) * sums.get(i));
		}
		/* Other way of getting sum by combining the 2 loops
		for (Projection p : outputs) {
			for (int i = 0; i < n; i++) {
				delta.set(i, derivativeVector.get(i) * (p.w.transpose().product(p.output.delta).sumOfElements()));
			}
		}
		*/
	}
